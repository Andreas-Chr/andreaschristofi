// Source: https://reactbits.dev/r/GradientWaves-JS-CSS.json
// Local additions: WebGL fallback and reduced-motion support.
...
const GradientWaves = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      return; // Keep the canvas background when WebGL is unavailable.
    }
    ...
    const tryStart = () => {
      if (!reducedMotion.matches && isVisible && isPageVisible && raf === 0) raf = requestAnimationFrame(loop);
    };
    const tryStop = () => {
      if (raf !== 0) {
    ...
    const onMotionChange = () => {
      reducedMotion.matches ? tryStop() : tryStart();
    };
